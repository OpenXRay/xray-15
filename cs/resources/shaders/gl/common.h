...
#  define xmaterial half(1.0f/4.f)
#else
...
struct         v_static                {
        ...
        float4      Nh                	;                // (nx,ny,nz,hemi occlusion)
        float4      T                 	;                // tangent
        float4      B                 	;                // binormal
        float2      tc                	;        // (u,v)
        float2      lmh                	;        // (lmu,lmv)
...
struct        v_detail                    {
        ...
        int4        misc                ;        // (u(Q),v(Q),frac,matrix-id)
};

#ifdef  USE_HWSMAP
struct         v_shadow_direct_aref
{
        float4      hpos;       // Clip-space position         (for rasterization)
        float2      tc0;       // Diffuse map for aref
};
...
struct         p_bumped        {
        ...
        float4            tcdh        ;        // Texture coordinates,         w=sun_occlusion
#else
        float2            tcdh        ;        // Texture coordinates
#endif
        float4      position        ;        // position + hemi
        half3       M1                ;        // nmap 2 eye - 1
        half3       M2                ;        // nmap 2 eye - 2
        half3       M3                ;        // nmap 2 eye - 3
#if defined(USE_PARALLAX) || defined(USE_STEEPPARALLAX)
        half3       eye                ;        // vector to point in tangent space
  #ifdef USE_TDETAIL
        float2      tcdbump     ;        // d-bump
    #ifdef USE_LM_HEMI
        float2      lmh                    ;        // lm-hemi
    #endif
  #else
...
#endif
        float4                position        ;        // position + hemi
        half3                N                ;        // Eye-space normal        (for lighting)
  #ifdef USE_TDETAIL
        float2                tcdbump                ;        // d-bump
    #ifdef USE_LM_HEMI
        float2         lmh      ;        // lm-hemi
...
//////////////////////////////////////////////////////////////////////////////////////////
struct                  f_deffer        		{
        half4           position        		;        // px,py,pz, m-id
        half4           Ne                		;        // nx,ny,nz, hemi
        half4       	C                		;        // r, g, b,  gloss
};
//////////////////////////////////////////////////////////////////////////////////////////
struct  				p_screen                {
        float4          hpos               		;
        float2          tc0                		;        // Texture coordinates         (for sampling maps)
};
//////////////////////////////////////////////////////////////////////////////////////////
...
uniform sampler2D       s_image;                // used in various post-processing
...
#define def_hdr         half(9.f)         		// hight luminance range half(3.f)
#define def_hdr_clip	half(0.75f)        		//
