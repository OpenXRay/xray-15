...
assembleAnimCurve (EtReadKey *firstKey, EtInt numKeys, EtBoolean isWeighted, EtBoolean useOldSmooth)
{
	...
	while (nextKey != kEngineNULL) {
		...
		case kTangentFixed:
			outTanX = key->outWeight * cos (key->outAngle) * 3.0;
			outTanY = key->outWeight * sin (key->outAngle) * 3.0;
			break;
		case kTangentLinear:
					...
					while (continueReading && !endOfKeys) {
						...
						if (buffer[0] == '}') {
							...
							if (readKey->inTangentType == kTangentFixed) {
								...
								readKey->inWeight = engineFileReadFloat (animFile);
								if (mayaVersion >= 3.0) {
									// We need to compensate for the new angle writing method
									//
									readKey->inAngle = atan(tan(readKey->inAngle) * frameRate);
								}
							}
							if (readKey->outTangentType == kTangentFixed) {
								readKey->outAngle = engineFileReadFloat (animFile) * angularConversion;
								readKey->outWeight = engineFileReadFloat (animFile);
								if (mayaVersion >= 3.0) {
									// We need to compensate for the new angle writing method
									//
									readKey->outAngle = atan(tan(readKey->outAngle) * frameRate);
								}
							}
