	...
	struct dgs_material	*m)		/* textured material paras */
{
	...
	*result = solid_black;
	...
	if (transp > 0 && diff > 0) {
		...
		addprod(result, &color, &m->diffuse, transp / M_PI);
	}
	if (refl > 0 && diff > 0) {
	        mi_compute_irradiance(&color, state);
		addprod(result, &color, &m->diffuse, refl / M_PI);
	}
        if (transp > 0 && spec > 0) {
...
extern "C" DLLEXPORT int dgs_material_version(void) {return(2);}
...
extern "C" DLLEXPORT int dgs_material_photon_version(void) {return(2);}
