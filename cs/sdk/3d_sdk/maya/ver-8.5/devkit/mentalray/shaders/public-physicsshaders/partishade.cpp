	...
	struct parti_volume	*paras)
{
	...
	return(2.0 * paras->extinction * texintens);
}
	...
	float			d)
{
	...
	if (paras->mode == 0) {		/* mode 0: fog everywhere in volume */
		if (paras->nonuniform == 0.0)	/* homogeneous medium */
			return(paras->extinction * state->dist);
		else {				/* nonhomogeneous medium */
			ks = 0;
		...
		s = (state->org.y + d * state->dir.y - paras->height) /
								state->dir.y;
		return(s > 0.0 ? paras->extinction * s : 0.0);
	}
}
	...
	float			d)		/* distance along the ray */
{
	...
	result->r = 1/(4*M_PI) * scatter.r * illum.r;
	result->g = 1/(4*M_PI) * scatter.g * illum.g;
	result->b = 1/(4*M_PI) * scatter.b * illum.b;
	...
	result->r += 1/(4*M_PI) * scatter.r / k * irrad.r;
	result->g += 1/(4*M_PI) * scatter.g / k * irrad.g;
	result->b += 1/(4*M_PI) * scatter.b / k * irrad.b;
	...
	Paras			*parms)
{
	...
	if (k > 0.0) {
		...
		result->r += back.r * step * jitter;
		result->g += back.g * step * jitter;
		result->b += back.b * step * jitter;
	} else
		back = black;
	...
	step = d + step - 2 * miEPS;
	if (step > 0.0) {
		k = density_func(state, paras, miEPS);
		if (k > 0.0) {
			compute_radiance(&front, state, paras, k, miEPS);
			if (uniform)
				attenuate_radiance_nosampling(result, state,
					paras, miEPS, step, 0, 
					&front, &back, fl);
	...
	if (!paras) {					/* shader init */
		*inst_req = miTRUE;
		...
		for (i=0; i < paras->iteration; i++) {
			...
			tw    += weight;
			scale += scale;
			weight = 0.5 * pow(scale, -paras->power);
		}
	...
	miState		*state)
{
	...
	if (sum > 1.0)
		sum = 1.0;
	return(lookup_color(tex->color_map, sum * sum * (3.0 - sum - sum)));
}
	...
	float	z)
{
	...
	u_2_0 = 0.5 * fr_x * fr_x;
	u_0_0 = 0.5 - fr_x + u_2_0;
	u_1_0 = 1.0 - u_0_0 - u_2_0;
	u_2_1 = 0.5 * fr_y * fr_y;
	u_0_1 = 0.5 - fr_y + u_2_1;
	u_1_1 = 1.0 - u_0_1 - u_2_1;
	u_2_2 = 0.5 * fr_z * fr_z;
	u_0_2 = 0.5 - fr_z + u_2_2;
	u_1_2 = 1.0 - u_0_2 - u_2_2;
