...
typedef enum miSpectrum_representation {
	miSPECTRUM_GRID		= 1,	/* equidistant samples */
	miSPECTRUM_SAMPLES	= 2, /* non-equidistant samples */

	miSPECTRUM_TYPE		= 0x00ff, /* up to 255 spectr. represent. */
	miSPECTRUM_FORCE	= 0x0100, /* if set transform file repres. */
	miSPECTRUM_READ		= 0x0200, /* set if spec is on disk */
	miSPECTRUM_FLAGS	= 0xff00  /* flags within repres. */  
} miSpectrum_representation;

typedef struct miSpectrum {
	miColor				tristimulus;	/* allow miSpectrum to
							 * be used as miColor.
							 * must be first member
							 */
	miSpectrum_representation	representation; 
	miTag				filename;	/* spectrum on disk */
	miTag				spec_tag;	/* spec load job */
	int				n_coeff;
	miScalar			min_wavelength;	/* in nano-meters */
	miScalar			max_wavelength; /* in nano-meters */
	miScalar			coeff[1];	/* may be larger  */
} miSpectrum;
...
miBoolean    mi_api_spectrum_path_config(char *, char *, char *);
miSpectrum  *mi_api_spectrum_begin	(char *name);
miTag	     mi_api_spectrum_end	(void);
miColor_profile *mi_api_colorprofile_begin(char *name);
