	...
	mib_glossy_reflection_t *param)
{
	...
	miColor   environment;
        ...
        while (samples && mi_sample(sample, &count, state, 2, &samples))   
        {
            miColor  thiscolor;
            miVector thisdir;
            miScalar angle = sample[0] * 2.0 * M_PI;          
            miScalar u_dev = sin(angle) * sqrt(sample[1]);
            miScalar v_dev = cos(angle) * sqrt(sample[1]); 
            ...
            /* Use mr reflection dir instead */
            mi_reflection_dir(&thisdir, state);
            
            actualsamples++;

            /* Edge multiplier */
            edge_level = pow(1.0 + (dot_nd<0.0?dot_nd:0.0), 
                            (double) edge_factor);

            weight = edge_weight * edge_level + base_weight * (1.0 - edge_level);
            ...
            if (mi_trace_reflection(&thiscolor, state, &thisdir))
            {
                ...
                else
                {
                    ...
                    if (reflection_depth > 0.0)
                    {
                        factor = 1.0 - (depth / reflection_depth);
                    ...
                    if (!sing_env)
                    {
                        environment.r += ref.r * (1.0 - factor) * weight;
                        environment.g += ref.g * (1.0 - factor) * weight;
                        environment.b += ref.b * (1.0 - factor) * weight;
                    }
                }
            }
            else /* Missed ray? */
        ...
        if (sing_env && environment_weight > 0.0)
        {
            ...
            /* Edge multiplier */
            edge_level = pow(1.0 + (state->dot_nd<0.0?state->dot_nd:0.0), 
                            (double) edge_factor);
	...
	mib_glossy_refraction_t *param)
{
	...
	miColor   deep_surface;
        ...
        if (ior > 0.0 && state->inv_normal) ior = 1.0 / ior;
        ...
        while (samples && mi_sample(sample, &count, state, 2, &samples))   
        {
            ...
            if (mi_trace_refraction(&thiscolor, state, &thisdir))
            {
                ...
                else
                {
                    ...
                    if (refraction_depth > 0.0)
                    {
                        factor = 1.0 - (depth / refraction_depth);
                        /* Tweak fallof by power funcion */
                        factor = pow(factor, falloff);
                    }

                    deep_surface_weight += (1.0 - factor);
