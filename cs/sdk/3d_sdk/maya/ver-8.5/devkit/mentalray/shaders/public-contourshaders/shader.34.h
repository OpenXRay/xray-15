...
typedef unsigned __int64		miIntptr;	/* for a pointer. Get*/
#else							/* this: 64-bit NT   */
...
typedef enum {
	...
	miM_RCRM,		/* rendering core: rapid motion */
...
typedef enum {
	...
	miIMG_FORMAT_ST_FP	= 40,	/* mi Color Texture, 16 bits */
...
typedef struct miImg_file {
	...
	int		spare[13];	/* makes sizeof(miImg_file) 256 bytes */
} miImg_file;
...
miUint1 mi_renderspace_cid(
		struct miState		*state);

miBoolean mi_ciexyz_to_internal(
		miColor			*c);		/* color to convert */

miBoolean mi_internal_to_ciexyz(
		miColor			*c);		/* color to convert */

miBoolean mi_internal_to_renderspace(
		struct miState		*state,
		miColor			*c);		/* color to convert */

miBoolean mi_renderspace_to_internal(
		struct miState		*state,
		miColor			*c);		/* color to convert */

miBoolean mi_ciexyz_to_renderspace(
		struct miState		*state,
		miColor			*c);		/* color to convert */

miBoolean mi_renderspace_to_ciexyz(
		struct miState		*state,
		miColor			*c);		/* color to convert */
...
typedef struct miOptions {
	...
	char		rapid_collect_rate;	/* rapid motion only: # of */
						/* fragment samples per pixel*/
						/* 0=use 1<<max_samples (def)*/
	char		rapid_motion_resample;	/* rapid motion only: # of */
	...
	miUint		spare2[30];		/* keep ray33 offsets */
	...
	miUint1		spare0[2];		/* not used */
	miTag		state_func;		/* allow user to init state */
	float		fg_presamples;		/* min.samples for FG precomp*/
	int		spare[12];		/* not used */
} miOptions;
typedef struct miCamera {
	...
	float		focus;			/* rapid motion: focus plane */
	float		radius;			/* rapid motion: lens radius */
