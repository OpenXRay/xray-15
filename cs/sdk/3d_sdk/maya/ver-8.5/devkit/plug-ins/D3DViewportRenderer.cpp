...
//
// Please contact original authors for any code change contributions.
//
// Main contact: BernardKwok@autodesk.com
// Additional contacts: GordonBradley@autodesk.com, CoryMogk@autodesk.com (non-code stuff).
//
#endif
...
D3DViewportRenderer::initialize()
{
	...
	if (m_pD3DDevice)
	{
		...
		if (builtRenderTargets)
		{
			...
			MString shaderLocation(MString(getenv("MAYA_LOCATION")) + MString("\\bin"));
			m_resourceManager.initializePostEffects( shaderLocation, m_pD3DDevice );
						...
						if (m_readBackBufferWidth && m_readBackBufferHeight)
						{
							...
							else
							{
								target.writeColorBuffer( m_readBackBuffer, 
									targetW/2 - m_readBackBufferWidth/2,
									targetH/2 - m_readBackBufferHeight/2);
							}
							status = MStatus::kSuccess;
						}
					}
...
//
{
	...
	if (hres == D3D_OK)
	{
		// Setup camera and world matrices
		setupMatrices();
...
bool D3DViewportRenderer::setupMatrices()
//
// Description:
//
...
//
{
    ...
    // World matrix is not set up here. It is done during
	// object drawing...
    //m_pD3DDevice->SetTransform( D3DTS_WORLD, &m_matWorld );

	CameraItem *cameraItem = m_resourceManager.getCamera();

    // Set up our view matrix. 
    D3DXMATRIXA16 matView;
	D3DXMatrixLookAtRH( &matView, &(cameraItem->m_vEyePt), &(cameraItem->m_vLookatPt), &(cameraItem->m_vUpVec) );
    m_pD3DDevice->SetTransform( D3DTS_VIEW, &matView );

    // Set up the projection matrix. This is currently only creates a perpective transform.
	// TO DO: 
	//	- Add orthographic camera support. 
	//
    D3DXMATRIXA16 matProj;
    D3DXMatrixPerspectiveFovRH( &matProj, D3DXToRadian(cameraItem->m_FieldOfView), 
						(float) m_renderWidth / (float)m_renderHeight, cameraItem->m_nearClip, cameraItem->m_farClip );
...
//
{
	...
	unsigned int numEffects = postEffectList.size();
