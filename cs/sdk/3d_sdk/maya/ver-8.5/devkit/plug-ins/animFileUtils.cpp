...
//
{
	...
	while (!readAnim.eof()) {
		...
		if (strcmp(dataType, kAnimVersion) == 0) {
			...
			if (version != thisVersion) {
				MString warnStr("Reading a version ");
				warnStr += version;
				warnStr += " file with a version ";
				warnStr += thisVersion;
				warnStr += " animImport plugin.";
				MGlobal::displayWarning(warnStr);
			}
	...
	//
	if (!hasVersionString) {
		MString errorStr("Missing a required keyword: ");
		errorStr += kAnimVersion;
		MGlobal::displayError(errorStr);
		return (MS::kFailure);
	}
		...
		if (strcmp(dataType, kAnim) == 0) {
			...
			if (strcmp(dataType, kAnimData) == 0) {
				...
				if (readAnimCurve(readAnim, clipboardItem)) {
					...
					//
					MGlobal::displayError("Could not read an anim curve.");
				}
			} else {
			...
			if (!readAnim.eof()) {
				...
				warnStr += " is an unrecognized keyword.";
				MGlobal::displayWarning(warnStr);
	...
	if (MS::kSuccess != cb.set(	clipboardArray, 
		...
		MGlobal::displayError("Could not add the anim curves to the clipboard.");
	}
...
//
{
	...
	while (!clipFile.eof()) {
		...
		if (strcmp(dataType, kInputString) == 0) {
			...
			if (!animUnitNames::setFromName(tUnit, tanAngleUnit)) {
				...
				MString warnStr("Setting the tanAngleUnit to ");
				warnStr += unitName;
				MGlobal::displayWarning(warnStr);
			}
	...
	if (status != MS::kSuccess) {
		MGlobal::displayError("Could not create the anim curve node.");
		return false;
	}
	...
	while (clipFile && c != kBraceRightChar) {
		...
		switch (type) {
			...
			case MFnAnimCurve::kAnimCurveUT:
				...
				break;
			default:
				MGlobal::displayError("Encountered an unknown anim curve type.");
				return false;
		}

		if (status != MS::kSuccess) {
			MGlobal::displayError("Could not add a keyframe.");
		}
	...
	if (clipFile.peek() == kBraceRightChar) {
		...
		//	Something is wrong.
		//
		MGlobal::displayError("Did not find an expected '}'");
	}
...
//
{
	...
	if (MS::kSuccess != status) {
		MGlobal::displayError("Could not read the anim curve for export.");
		return false;
	}
