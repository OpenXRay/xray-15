...
of the plugin using Microsoft VC7 (2003), as well as the 64-bit version of the 
plugin using Microsoft VC8 (2005). The DirectX Developer SDK compiled with is the 
April, 2006 release from Microsoft. The include and link library directories will 
...
If the SDK has not been installed then the  DirectX End-User Runtimes (June 2006) must be 
installed as the plugin is dependent on these libraries. Make sure that version 30 of 
Direct3d is installed (e.g. check the windows/system32 directory for d3dx9_30.dll).
