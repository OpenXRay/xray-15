...
void    
cgfxGetFxIncludePath( const MString &fxFile, MString &pathOptions )
{
	...
	if (fxFile.length())
	{
		...
		pathOptions = MString("-I") + fobject.resolvedPath();		
	}
	else
		pathOptions.set("");
	...
	if (cgfxRoot)
	{
		pathOptions += 	MString(";") + MString(cgfxRoot);
		pathOptions += 	MString(";") + MString(cgfxRoot) + MString("/CgFX");
	}
}
