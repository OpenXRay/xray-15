	...
	struct mib_texture_vector *paras)
{
	...
	const miVector	*vp;		/* pointer to working vector */
	miVector	tmp;		/* temporaries */
	...
	struct mib_bump_basis *paras)
{
	...
	const miVector	*v0, *v1, *v2;	/* triangle points in object space */
	const miVector	*t0, *t1, *t2;	/* triangle texture verts */
	miVector	norm_obj;	/* normal in object space */
	miVector	refdir;
	...
	switch (project) {
	  ...
	  case TP_UV: {						/* UV */
		...
		if (have_v) {
			...
			mi_vector_prod(&result->u, &norm_obj, &result->v);
			...
			mi_vector_normalize(&result->u);
			mi_vector_prod(&result->v, &result->u, &norm_obj);
			/* The result should be less than 90 degrees
			   from refdir. if it isn't, flip it */
