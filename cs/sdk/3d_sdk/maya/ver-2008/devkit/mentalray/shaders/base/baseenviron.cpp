	...
	if (fabs(dir->x) >= EPS) {
		...
		if (s >= 0.0) {
			...
			if (x.y >= -size->y && x.y <= size->y &&
				...
				coord->x = 1.0F - 0.5F * (x.y / size->y + 1.0F);
				coord->y = 0.5F * (x.z / size->z + 1.0F);
				coord->z = 0.0;
				return(TEX_PX);
			}
		}
	...
	if (fabs(dir->y) >= EPS) {
		...
		if (s >= 0.0) {
			...
			if (x.x >= -size->x && x.x <= size->x &&
				...
				coord->x = 1.0F - 0.5F * (x.x / size->x + 1.0F);
				coord->y = 0.5F * (x.z / size->z + 1.0F);
				coord->z = 0.0;
				return(TEX_MZ);
			}
	...
	miScalar	end)
{
	...
	if (x == 0.)
		tex->x = (y >= 0.) ? M_PI_F/2.F : 3.F*M_PI_F/2.F;
	else {
		if (y >= 0.) {
			if (x > 0.)
				tex->x = (miScalar)atan(y/x);		/* 0..90 */
			else
				tex->x = M_PI_F - (miScalar)atan(y/(-x));	/* 90..180 */
		} else {
			if (x > 0.)
				tex->x = 2.F*M_PI_F - (miScalar)atan(-y/x);	/* 270..360 */
			else
				tex->x = M_PI_F + (miScalar)atan(y/x);	/* 180..270 */
		}
	}
