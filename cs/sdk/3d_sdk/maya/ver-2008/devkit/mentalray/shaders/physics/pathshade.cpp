	...
	miScalar		*ior_out)
{
	...
	if (!(num & 1)) {					/* entering */
		...
		*ior_in  = state->parent && state->parent->ior != 0.0 ?
						state->parent->ior : 1.0F;
		if (!state->refraction_volume)
			state->refraction_volume = state->volume;
	} else {						/* exiting */
		*ior_in  = paras->ior;
		*ior_out = (s_in && s_in->ior_in != 0.0) ? s_in->ior_in : 1.0F;
	...
	miTag			*light)
{
	...
	for (mi::shader::LightIterator iter(state, light, m->n_light); 
		...
		while (iter->sample()) {
			...
			if (diff) {
				f = dot_nl * refl;
				sum.r += f * m->diffuse.r * (1.F/M_PI_F) * color.r;
				sum.g += f * m->diffuse.g * (1.F/M_PI_F) * color.g;
				sum.b += f * m->diffuse.b * (1.F/M_PI_F) * color.b;
			}
...
int path_material_version(void) {return(2);}
	...
	int		*paras)		/* number of samples */
{
	...
	for (i=0; i < *paras; i++) {
		...
		sum.r += mi_MIN(result->r, 1.0F);
		sum.g += mi_MIN(result->g, 1.0F);
		sum.b += mi_MIN(result->b, 1.0F);
		sum.a += mi_MIN(result->a, 1.0F);
	}
