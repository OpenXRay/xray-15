...
extern "C" DLLEXPORT int dielectric_material_photon_version(void) {return 2;}
...
extern "C" DLLEXPORT int dielectric_material_version(void) {return 2;}
	...
	*res = black;
	...
	if (mc.pcoef > 0) {
		...
		if (mode == 4) {
			...
			lgt	=  mi_eval_tag    (paras->light) + i_light;

			if (mode == 1)		/* modify light list (inclusive mode) */
				mi_inclusive_lightlist(&n_light, &lgt, state);
			else if (mode == 2)	/* modify light list (exclusive mode) */
				mi_exclusive_lightlist(&n_light, &lgt, state);
		}

		if (mode == 4 || n_light) {
			for (mi::shader::LightIterator iter(state, lgt, n_light);
			                               !iter.at_end(); ++iter) {
				miColor		csum;
				miScalar	ns;
