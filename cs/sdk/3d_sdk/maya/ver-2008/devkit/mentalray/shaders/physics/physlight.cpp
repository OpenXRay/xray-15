	...
	struct physical_light	*parms)
{
	...
	miScalar		cosine, r2, f, d, area, radius;
	miScalar		r=(miScalar)state->dist;
	...
	if (state->type == miRAY_LIGHT) {
		...
		switch(areatype) {
		  case miLIGHT_NONE:		/* point, spot or directional*/
			 ...
			 * spherical light
			 */
			f = type==miLIGHT_DIRECTION ? 1.F : 1.F / (4.F * M_PI_F * r2);
			break;
		...
		switch (areatype) {
		  case miLIGHT_RECTANGLE:	/* rectangular area light */
			...
			if (mi_vector_dot(&normal, &dir) > 0) {
				area = 4.0F * mi_vector_norm(&normal);
				miASSERT(area > 0.0);
		  ...
		  case miLIGHT_SPHERE:		/* spherical area light */
			...
			area = 4.0F * M_PI_F * radius * radius;
			miASSERT(area > 0.0);
			result->r = paras->color.r / area;
			result->g = paras->color.g / area;
			result->b = paras->color.b / area;
		  ...
		  case miLIGHT_CYLINDER:	/* cylindrical area light */
			...
			area = 2.0F * M_PI_F * radius * radius *
						mi_vector_norm(&axis);
			miASSERT(area > 0.0);
			result->r = paras->color.r / area;
			result->g = paras->color.g / area;
