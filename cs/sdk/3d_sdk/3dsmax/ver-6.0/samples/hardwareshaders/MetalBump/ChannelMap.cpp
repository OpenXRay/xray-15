...
void ChannelMap::SetTexture(LPDIRECT3DDEVICE8 Device, bool AlphaOn)
{
	...
	if(m_Movie)
	{
		Device->SetTextureStageState(m_Stage,D3DTSS_MIPFILTER,D3DTEXF_NONE);
	}
...
IDirect3DBaseTexture8* ChannelMap::GetTexture()
{
	return(m_Texture);
}
...
bool ChannelMap::LoadTexture(LPDIRECT3DDEVICE8 Device)
{		
	bool		Ret;
		...
		{ 
			...
			if(m_Movie->Load(m_Name,Width,Height,Bpp,24))
			{
				...
				Hr = Device->CreateTexture(Width,Height,
										   1,0,Format,
										   D3DPOOL_MANAGED,(IDirect3DTexture8 **)&m_Texture);
		...
		else
		{
			switch(m_Channel)
			{
				...
				case CHANNEL_MASK:
										...
										if(Ret)
										{
											GrayScaleAlpha((IDirect3DTexture8 *)m_Texture);
										}	
...
bool ChannelMap::LoadSingleTexture(LPDIRECT3DDEVICE8 Device, bool MipMap)
{		
	TCHAR   *MapPath;
	HRESULT  Hr;
	int		 NumMip,Index;
...
bool ChannelMap::LoadSpecTexture(LPDIRECT3DDEVICE8 Device)
{		
	HRESULT			Hr;
	int				Index;
	void			*Data;
...
bool ChannelMap::LoadNormalTexture(LPDIRECT3DDEVICE8 Device)
{		
	TCHAR   *MapPath;
	HRESULT  Hr;
	int		 Index;
	...
	if(Index < 0)
	{
		...
		if(Hr == S_OK)
		{
			ConvertToBias(Device,(IDirect3DTexture8 *)m_Texture);
			TextureMgr::GetIPtr()->Add(m_Name,m_Type,m_Texture);
...
bool ChannelMap::LoadCubeTexture(LPDIRECT3DDEVICE8 Device)
{
	TCHAR   *MapPath;
	HRESULT  Hr;
	int		 Index;
	...
	if(Index < 0)
	{
		...
		Hr = D3DXCreateCubeTextureFromFileExA(Device,
											  ...
											  NULL,
											  NULL,
											  (IDirect3DCubeTexture8 **)&m_Texture);
...
bool ChannelMap::LoadBumpTexture(LPDIRECT3DDEVICE8 Device)
{
	TCHAR				*MapPath;
	HRESULT				Hr;
...
unsigned char *ChannelMap::GetBitmapPtr(int &Pitch, int &Height)
{
	...
	if(m_Texture)
	{
		((LPDIRECT3DTEXTURE8)m_Texture)->GetLevelDesc(0,&Desc);

		Hr = ((LPDIRECT3DTEXTURE8)m_Texture)->LockRect(0,&Rect,NULL,0);
...
unsigned long ChannelMap::GetBitmapPixel(float U, float V)
{
	...
	if(m_Texture)
	{
		Tex = (LPDIRECT3DTEXTURE8) m_Texture;
...
void ChannelMap::ReleaseBitmapPtr()
{
	if(m_Texture)
	{
		((LPDIRECT3DTEXTURE8)m_Texture)->UnlockRect(0);
	}
...
bool ChannelMap::ConvertToBias(LPDIRECT3DDEVICE8 Device, LPDIRECT3DTEXTURE8 Source)
{
	LPDIRECT3DTEXTURE8	NormalMap;
	LPDIRECT3DSURFACE8	SourceTex,DestTex;
	...
	Hr = Device->CreateTexture(DescSource.Width, DescSource.Height,
							   ...
							   D3DFMT_Q8W8V8U8, D3DPOOL_MANAGED, &NormalMap);
	if(FAILED(Hr))
	{
		return(false);
	}
