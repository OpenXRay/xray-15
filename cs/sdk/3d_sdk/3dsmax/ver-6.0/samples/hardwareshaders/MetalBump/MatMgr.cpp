...
void MatMgr::LoadDefaults(LPDIRECT3DDEVICE8 Device, bool FromResource)
{
	int		i;
	HRESULT Hr;
	TSTR	Str;
		...
		if(FromResource)
		{
			for(i=0; i < m_NumMat; i++)
			{
				...
				if(m_MatInfos[i].m_VertexFile.Length())
				{
					if(!FindVertexFile(i))
					{
						if(FAILED(Hr = CreateShaderResource(Device,m_MatInfos[i].m_VertexRes,gVertexDecl,
				...
				if(m_MatInfos[i].m_PixelFile.Length())
				{
					if(!FindPixelFile(i))
					{
						if(FAILED(Hr = CreateShaderResource(Device,m_MatInfos[i].m_PixelRes,NULL,
				...
				if(m_MatInfos[i].m_AlphaPixelFile.Length())
				{
					...
					{
						if(FAILED(Hr = CreateShaderResource(Device,m_MatInfos[i].m_AlphaPixelRes,NULL,
