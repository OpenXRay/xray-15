...
PredictorPrintDir(TIFF* tif, FILE* fd, long flags)
{
	...
	if (TIFFFieldSet(tif,FIELD_PREDICTOR)) {
		fprintf(fd, GetString(IDS_PREDICTOR_COLON));//"  Predictor: ");
		switch (sp->predictor) {
		case 1: fprintf(fd, GetString(IDS_PRED_NONE)/*"none "*/); break;
		case 2: fprintf(fd, GetString(IDS_PRED_HORIZ)/*"horizontal differencing "*/); break;
		}
