...
TIFFRGBAImageOK(TIFF* tif, char emsg[1024])
{
	...
	break;
    default:
	sprintf(emsg, GetString(IDS_BAD_BIT_SAMPS),//"Sorry, can not handle images with %d-bit samples",
	    td->td_bitspersample);
	return (0);
    }
	...
	case 3:
	    ...
	    break;
	default:
	    sprintf(emsg, GetString(IDS_MISS_NEED)/*"Missing needed %s tag"*/, photoTag);
	    return (0);
	}
    }
    switch (photometric) {
	...
	if (td->td_planarconfig != PLANARCONFIG_CONTIG) {
	    sprintf(emsg, GetString(IDS_BADYCBCR),//"Sorry, can not handle YCbCr images with %s=%d",
	...
	if (colorchannels < 3) {
	    sprintf(emsg, GetString(IDS_RGB_CHAN),//"Sorry, can not handle RGB image with %s=%d",
	...
	if (td->td_inkset != INKSET_CMYK) {
	    sprintf(emsg, GetString(IDS_SEPAR_WITH),//"Sorry, can not handle separated image with %s=%d",
		"InkSet", td->td_inkset);
	    return (0);
	}
	...
	if (td->td_compression != COMPRESSION_SGILOG) {
	    sprintf(emsg, GetString(IDS_LOGL_DATA),//"Sorry, LogL data must have %s=%d",
	...
	if (td->td_compression != COMPRESSION_SGILOG &&
	    ...
	    sprintf(emsg, GetString(IDS_LOGLUV_DATA),//"Sorry, LogLuv data must have %s=%d or %d",
		GetString(IDS_COMPRESSION), COMPRESSION_SGILOG, COMPRESSION_SGILOG24);
	    return (0);
	}
	if (td->td_planarconfig != PLANARCONFIG_CONTIG) {
	    sprintf(emsg, GetString(IDS_WACKED_OUT_LOGLUV),//"Sorry, can not handle LogLuv images with %s=%d",
		"Planarconfiguration", td->td_planarconfig);
	    return (0);
	}
	break;
    default:
	sprintf(emsg, GetString(IDS_BAD_IMAGE),//"Sorry, can not handle image with %s=%d",
	    photoTag, photometric);
	return (0);
    }
    return (1);
...
TIFFRGBAImageBegin(TIFFRGBAImage* img, TIFF* tif, int stop, char emsg[1024])
{
	...
	break;
    default:
	sprintf(emsg, GetString(IDS_SORRY_BIT_SAMPS),//"Sorry, can not image with %d-bit samples",
	    img->bitspersample);
	return (0);
    }
    img->alpha = 0;
	...
	break;
    default:
	sprintf(emsg, GetString(IDS_OH_NO),//"Sorry, can not handle image with %s=%d",
	    photoTag, img->photometric);
	return (0);
    }
    img->Map = NULL;
