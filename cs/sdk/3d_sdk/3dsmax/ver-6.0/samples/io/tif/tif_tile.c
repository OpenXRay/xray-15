...
TIFFCheckTile(TIFF* tif, uint32 x, uint32 y, uint32 z, tsample_t s)
{
	...
	if (x >= td->td_imagewidth) {
		TIFFError(tif->tif_name, GetString(IDS_TILE_COL_OOR),//"Col %ld out of range, max %lu",
		    (long) x, (u_long) td->td_imagewidth);
		return (0);
	}
	if (y >= td->td_imagelength) {
		TIFFError(tif->tif_name, GetString(IDS_TILE_ROW_OOR),//"Row %ld out of range, max %lu",
		    (long) y, (u_long) td->td_imagelength);
		return (0);
	}
	if (z >= td->td_imagedepth) {
		TIFFError(tif->tif_name, GetString(IDS_TILE_DEPTH_OOR),//"Depth %ld out of range, max %lu",
		    (long) z, (u_long) td->td_imagedepth);
		return (0);
	}
	if (td->td_planarconfig == PLANARCONFIG_SEPARATE &&
	    s >= td->td_samplesperpixel) {
		TIFFError(tif->tif_name, GetString(IDS_TILE_SAMPLE_OOR),//"Sample %d out of range, max %u",
