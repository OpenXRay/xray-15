...
ZIPDecode(TIFF* tif, tidata_t op, tsize_t occ, tsample_t s)
{
	...
	do {
		...
		if (state == Z_DATA_ERROR) {
			TIFFError(module,
			    GetString(IDS_ZIP_DECODE_ERR),//"%s: Decoding error at scanline %d, %s",
...
ZIPEncode(TIFF* tif, tidata_t bp, tsize_t cc, tsample_t s)
{
	...
	do {
		if (deflate(&sp->stream, Z_NO_FLUSH) != Z_OK) {
			TIFFError(module, GetString(IDS_ZIP_ENCOD_ERR),//"%s: Encoder error: %s",
			    tif->tif_name, sp->stream.msg);
			return (0);
		}
		if (sp->stream.avail_out == 0) {
...
TIFFInitZIP(TIFF* tif, int scheme)
{
	...
	return (1);
bad:
	TIFFError("TIFFInitZIP", GetString(IDS_ZIP_NO_SPACE));//"No space for ZIP state block");
	return (0);
}
#endif /* ZIP_SUPORT */
