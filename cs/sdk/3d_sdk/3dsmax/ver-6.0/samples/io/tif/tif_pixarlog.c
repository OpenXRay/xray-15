...
PixarLogSetupDecode(TIFF* tif)
{
	...
	if (sp->user_datafmt == PIXARLOGDATAFMT_UNKNOWN) {
		TIFFError(module, 
			GetString(IDS_PIXAR_BAD_DEPTH),//"PixarLog compression can't handle bits depth/data format combination (depth: %d)", 
			td->td_bitspersample);
		return (0);
	}
...
PixarLogDecode(TIFF* tif, tidata_t op, tsize_t occ, tsample_t s)
{
	...
	default:
		TIFFError(tif->tif_name,
			GetString(IDS_N_BIT_INPUT_PIXAR),//"%d bit input not supported in PixarLog",
			td->td_bitspersample);
		return 0;
	}
	...
	do {
		...
		if (state == Z_DATA_ERROR) {
			TIFFError(module,
			    GetString(IDS_PIXAR_DECODE_ERR),//"%s: Decoding error at scanline %d, %s",
		...
		if (state != Z_OK) {
			TIFFError(module, GetString(IDS_ZLIB_ERR),//"%s: zlib error: %s",
	...
	if (sp->stream.avail_out != 0) {
		TIFFError(module,
		    GetString(IDS_PIXAR_NED),//"%s: Not enough data at scanline %d (short %d bytes)",
	...
	for (i = 0; i < nsamples; i += llen, up += llen) {
		...
		default:
			TIFFError(tif->tif_name,
				  GetString(IDS_PIXAR_UNSUPP_BPS),//"PixarLogDecode: unsupported bits/sample: %d", 
				  td->td_bitspersample);
			return (0);
		}
	}
...
PixarLogSetupEncode(TIFF* tif)
{
	...
	if (sp->user_datafmt == PIXARLOGDATAFMT_UNKNOWN)
		sp->user_datafmt = PixarLogGuessDataFmt(td);
	if (sp->user_datafmt == PIXARLOGDATAFMT_UNKNOWN) {
		TIFFError(module, GetString(IDS_PIXAR_N_BIT_LIN),/*"PixarLog compression can't handle %d bit linear encodings"*/
			td->td_bitspersample);
		return (0);
	}
...
PixarLogEncode(TIFF* tif, tidata_t bp, tsize_t cc, tsample_t s)
{
	...
	do {
		if (deflate(&sp->stream, Z_NO_FLUSH) != Z_OK) {
			TIFFError(module, GetString(IDS_PIXAR_ENC_ERR),//"%s: Encoder error: %s",
			    tif->tif_name, sp->stream.msg);
			return (0);
		}
		if (sp->stream.avail_out == 0) {
     ...
     case TIFFTAG_PIXARLOGQUALITY:
		...
		if (tif->tif_mode != O_RDONLY && (sp->state&PLSTATE_INIT)) {
			if (deflateParams(&sp->stream,
				...
				TIFFError(module, GetString(IDS_PIXAR_ENC_ERR),//"%s: zlib error: %s",
					tif->tif_name, sp->stream.msg);
				return (0);
			}
		}
...
TIFFInitPixarLog(TIFF* tif, int scheme)
{
	...
	return (1);
bad:
	TIFFError("TIFFInitPixarLog", GetString(IDS_PIXAR_NO_SPACE));//"No space for PixarLog state block");
	return (0);
}
