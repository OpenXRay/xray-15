...
TIFFjpeg_tables_dest(JPEGState* sp, TIFF* tif)
{
	...
	if (sp->jpegtables == NULL) {
		...
		TIFFError("TIFFjpeg_tables_dest", GetString(IDS_NO_SPACE_JPEG));//"No space for JPEGTables");
		return (0);
	}
	sp->cinfo.c.dest = &sp->dest;
...
JPEGSetupDecode(TIFF* tif)
{
	...
	if (TIFFFieldSet(tif,FIELD_JPEGTABLES)) {
		...
		if(TIFFjpeg_read_header(sp,FALSE) != JPEG_HEADER_TABLES_ONLY) {
			TIFFError("JPEGSetupDecode", GetString(IDS_BOGUS_JPEGTABLE));//"Bogus JPEGTables field");
			return (0);
		}
	}
...
JPEGPreDecode(TIFF* tif, tsample_t s)
{
	...
	if (sp->cinfo.d.image_width != segment_width ||
		...
		TIFFError(module, GetString(IDS_IMPROPER_STSIZE));//"Improper JPEG strip/tile size");
		return (0);
	}
	if (sp->cinfo.d.num_components !=
	    (td->td_planarconfig == PLANARCONFIG_CONTIG ?
	     td->td_samplesperpixel : 1)) {
		TIFFError(module, GetString(IDS_JPEG_COM_COUNT));//"Improper JPEG component count");
		return (0);
	}
	if (sp->cinfo.d.data_precision != td->td_bitspersample) {
		TIFFError(module, GetString(IDS_BAD_JPEG_PRECISION));//"Improper JPEG data precision");
		return (0);
	}
	if (td->td_planarconfig == PLANARCONFIG_CONTIG) {
		...
		if (sp->cinfo.d.comp_info[0].h_samp_factor != sp->h_sampling ||
			...
			TIFFError(module, GetString(IDS_JPEG_SAMPLING_FACTOR));//"Improper JPEG sampling factors");
			return (0);
		}
...
JPEGDecode(TIFF* tif, tidata_t buf, tsize_t cc, tsample_t s)
{
	...
	if (cc % sp->bytesperline)
		TIFFWarning(tif->tif_name, GetString(IDS_JPEG_SCANLINENR));//"fractional scanline not read");
...
JPEGSetupEncode(TIFF* tif)
{
	...
	case PHOTOMETRIC_MASK:
		TIFFError(module,
			  GetString(IDS_JPEG_BAD_PHOTO),//"PhotometricInterpretation %d not allowed for JPEG",
			  (int) sp->photometric);
		return (0);
	default:
		/* TIFF 6.0 forbids subsampling of all other color spaces */
	...
	if (td->td_bitspersample != BITS_IN_JSAMPLE) {
		TIFFError(module, GetString(IDS_BAD_JPEG_BPS),//"BitsPerSample %d not allowed for JPEG",
	...
	if (isTiled(tif)) {
		if ((td->td_tilelength % (sp->v_sampling * DCTSIZE)) != 0) {
			TIFFError(module,
				  GetString(IDS_JPEG_TH),//"JPEG tile height must be multiple of %d",
				  sp->v_sampling * DCTSIZE);
			return (0);
		}
		if ((td->td_tilewidth % (sp->h_sampling * DCTSIZE)) != 0) {
			TIFFError(module,
				  GetString(IDS_JPEG_TW),//"JPEG tile width must be multiple of %d",
		...
		if (td->td_rowsperstrip < td->td_imagelength &&
			...
			TIFFError(module,
				  GetString(IDS_RPS_JPEG),//"RowsPerStrip must be multiple of %d for JPEG",
				  sp->v_sampling * DCTSIZE);
			return (0);
		}
	}
...
JPEGPreEncode(TIFF* tif, tsample_t s)
{
	...
	if (segment_width > 65535 || segment_height > 65535) {
		TIFFError(module, GetString(IDS_JPEG_TOO_BIG));//"Strip/tile too large for JPEG");
		return (0);
	}
...
JPEGEncode(TIFF* tif, tidata_t buf, tsize_t cc, tsample_t s)
{
	...
	nrows = cc / sp->bytesperline;
	if (cc % sp->bytesperline)
		TIFFWarning(tif->tif_name, GetString(IDS_JPEG_SCANLINENR));//"fractional scanline discarded");
...
JPEGPrintDir(TIFF* tif, FILE* fd, long flags)
{
	...
	(void) flags;
	if (TIFFFieldSet(tif,FIELD_JPEGTABLES))
		fprintf(fd, GetString(IDS_JPEG_TABLS),//"  JPEG Tables: (%lu bytes)\n",
...
TIFFInitJPEG(TIFF* tif, int scheme)
{
	...
	if (tif->tif_data == NULL) {
		TIFFError("TIFFInitJPEG", GetString(IDS_JPEG_NO_SPC_STBLK));//"No space for JPEG state block");
		return (0);
	}
	sp = JState(tif);
