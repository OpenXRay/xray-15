...
void glEnvDescriptorRender::Copy(IEnvDescriptorRender &_in)
{
	*this = *(glEnvDescriptorRender*)&_in;
}

void glEnvDescriptorMixerRender::Copy(IEnvDescriptorMixerRender &_in)
{
	*this = *(glEnvDescriptorMixerRender*)&_in;
}

void glEnvironmentRender::Copy(IEnvironmentRender &_in)
{
	*this = *(glEnvironmentRender*)&_in;
}

particles_systems::library_interface const& glEnvironmentRender::particles_systems_library()
{
	return	(RImplementation.PSLibrary);
}

void glEnvDescriptorMixerRender::Destroy()
{
...
void glEnvDescriptorMixerRender::Clear()
{
...
void glEnvDescriptorMixerRender::lerp(IEnvDescriptorRender *inA, IEnvDescriptorRender *inB)
{
	glEnvDescriptorRender *pA = (glEnvDescriptorRender *)inA;
	glEnvDescriptorRender *pB = (glEnvDescriptorRender *)inB;
...
void glEnvDescriptorRender::OnDeviceCreate(CEnvDescriptor &owner)
{
...
void glEnvDescriptorRender::OnDeviceDestroy()
{
	sky_texture.destroy();
...
glEnvironmentRender::glEnvironmentRender()
{
	tsky0 = DEV->_CreateTexture("$user$sky0");
	tsky1 = DEV->_CreateTexture("$user$sky1");
}

void glEnvironmentRender::OnFrame(CEnvironment &env)
{
	glEnvDescriptorMixerRender &mixRen = *(glEnvDescriptorMixerRender*)&*env.CurrentEnv->m_pDescriptorMixer;

	if (::Render->get_generation() == IRender_interface::GENERATION_R2){
		// TODO: OGL: Implement tone mapping.
	}
	...
	//	TODO: OGL: Implement environment parameters setting for OGL (if necessary)
}

void glEnvironmentRender::OnLoad()
{
	tonemap = DEV->_CreateTexture("$user$tonemap");	//. hack
}

void glEnvironmentRender::OnUnload()
{
	tonemap = 0;
}

void glEnvironmentRender::RenderSky(CEnvironment &env)
{
...
void glEnvironmentRender::RenderClouds(CEnvironment &env)
{
	::Render->rmFar();
...
void glEnvironmentRender::OnDeviceCreate()
{
	sh_2sky.create(&m_b_skybox, "skybox_2t");
	sh_2geom.create(v_skybox_fvf, RCache.Vertex.Buffer(), RCache.Index.Buffer());
	clouds_sh.create("clouds", "null");
...
void glEnvironmentRender::OnDeviceDestroy()
{
