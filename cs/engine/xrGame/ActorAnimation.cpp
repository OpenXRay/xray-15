...
static const float p_spin0_factor		= 0.7f;//v2v3v4 гнЄм спину тут
...
void CActor::g_SetAnimation( u32 mstate_rl )
{
			...
			if (!b_DropActivated&&!fis_zero(f_DropPower)){
				...
				if (!m_bAnimTorsoPlayed) {
					if (W) {
						...
						if(K)
						{
							switch (W->GetState()){//v2v3v4 фикс анимок дл€ ножа
					...
					else if (M) {//v2v3v4 фикс анимок дл€ гранат и болта
							switch (M->GetState()){
	...
	if (!inventory().ActiveItem())
			...
			M_torso = ST->m_torso[6].moving[moving_idx];//v2v3v4 даЄм анимки безоружному здесь 
	...
	// есть анимаци€ дл€ всего - запустим / иначе запустим анимацию по част€м
