...
MStatus	parseShader(MObject &src, SXRShaderData& d)
{
	...
	if (status == MS::kSuccess){
		...
		if ((status == MS::kSuccess)&&(MS::kSuccess==xrEnginePlug.getValue(index))) d.eng_name = enm.fieldName(index);
	}
	//compiler
	...
	if (status == MS::kSuccess){
		...
		if ((status == MS::kSuccess)&&(MS::kSuccess==xrCompilerPlug.getValue(index))) d.comp_name = enm.fieldName(index);
	}
	//game material
	...
	if (status == MS::kSuccess){
		...
		if ((status == MS::kSuccess)&&(MS::kSuccess==xrGameMaterialPlug.getValue(index))) d.gmat_name = enm.fieldName(index);
	}
